...
import { CANDIDATE_BUNDLE_CAPABILITY } from "../../../source-skills/candidate-bundle.ts";
import type { ParsedAccountBalance, ParsedTransactions } from "../contracts.ts";
import { compileTransactionCandidates } from "../transaction-candidates.ts";
import type { HistoryRecoveryEvidence, VerifyReport } from "../verify.ts";
    ...
    async prepareFetch({ config, endDateEpoch, previousCapture, resume }) {
      ...
      return {
        ...
        compiledSource: {
          ...
          async compile({ bytes }) {
            const current = filterSimpleFinTransactions(
              await simpleFinParser.parse(bytes),
              normalizedConfig,
            );
            return compileTransactionCandidates(current, {
              verify: verifyOptions,
              identitySourceProperties: simpleFinIdentitySourceProperties,
            });
          },
          verificationError(report) {
            return simpleFinVerificationAction(report as unknown as VerifyReport);
          },
        },
...
function simpleFinIdentitySourceProperties(
