...
describe("SimpleFIN connected-source skill", () => {
  ...
  test("normalizes stored config and prepares an exact account request", async () => {
    ...
    const fetch = skill.prepareFetch({
      config,
    ...
    expect(fetch.verifyOptions).toEqual({});
  ...
  test("extends one request beyond 45 days and requires reviewed recovery past 90 days", () => {
    ...
    const skill = createSimpleFinSkill({ allowedHosts: [allowedHost] });
    const resumed = skill.prepareFetch({
      config: {},
      endDateEpoch,
      previous: stale,
      resume: { mode: "rebaseline" },
    ...
    expect(resumed.verifyOptions.historyRecovery).toMatchObject({ mode: "rebaseline" });
    expect(() =>
      skill.prepareFetch({
        config: {},
        endDateEpoch,
    ...
    ).toThrow("SimpleFIN continuation resume is invalid");
  });
  ...
  test("owns balance-snapshot identity projection and reviewed verification recovery", async () => {
    ...
    const previous = skill.normalize(await parseFixture("accounts-previous-v2.json"), {});
    const current = skill.normalize(await parseFixture("accounts-current-v2.json"), {});
    current.accountBalances![0] = {
      ...current.accountBalances![0]!,
      balance: "1125.51",
    };
    const report = verifyTransactions(current, { previous });
    const action = skill.verificationError(report);
...
async function parseFixture(name: string): Promise<ParsedTransactions> {
  ...
  return simpleFinParser.parse(bytes);
}
