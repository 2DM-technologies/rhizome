...
test("search combines commands and loaded Vibe titles", () => {
  const results = searchShell("", [{ uuid: VIBE_UUID, title: "Spending" }], "light");
  ...
  expect(
    searchShell("spend", [{ uuid: VIBE_UUID, title: "Spending" }], "light")[0]?.action,
  ).toEqual({
    kind: "open",
...
test("a loaded Vibe appears once in the Vibes section", () => {
  ...
  const results = searchShell("spending", [{ uuid: VIBE_UUID, title: "Spending" }], "light");
...
test("surface paths round-trip through the canonical URL parser", () => {
  ...
  expect(labelOf(importSurface)).toBe("Ingest");
