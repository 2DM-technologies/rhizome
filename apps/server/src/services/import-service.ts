...
import type { ParsedAccountBalance, ParsedTransactions } from "../../../ingest/src/types.ts";
import {
  ARENA_PARSER_VERSION,
  parseArenaCapture,
  type ParsedArenaChannel,
  type ParsedArenaElement,
} from "../../../ingest/skills/arena/scripts/parse-arena.ts";
import { verifyArena, type ArenaVerifyReport } from "../../../ingest/verify/arena.ts";
...
type SourceVerifyReport = VerifyReport | ArenaVerifyReport;
...
export interface ArenaChannelFetcher {
  fetchChannelCapture(channelSlug: string): Promise<Uint8Array>;
}
...
export class ImportService {
  ...
  private readonly arena: ArenaChannelFetcher;
  ...
  async confirm(vibeUuid: string, operationUuid: string): Promise<VibeAggregate> {
    ...
    await this.db.transaction(async (transaction: DatabaseTransaction) => {
      ...
      const sourceUuid = sourceUuidOf(sourceReference);
      ...
      const stagedOriginUuid = originUuidOf(result.staged_origin);
      ...
      const stagedFetch =
        resolved.kind === "credential" || resolved.kind === "remote"
  ...
  private async runPreview(operationUuid: string, sourceUuid: string, vibe: DbVibe): Promise<void> {
    ...
    const currentSourceDigest = staged.sourceStateDigest;
  ...
  private async stageSource(
    ...
    if (resolved.kind === "remote") {
      return this.stageRemoteSource(resolved, ownerUuid, operationUuid);
    }
    return this.stageOriginSource(resolved, ownerUuid);
  }

  private async stageRemoteSource(
    resolved: ResolvedRemoteSource,
    ownerUuid: string,
    operationUuid: string,
  ): Promise<StagedSourceCapture> {
    if (
      resolved.source.provider !== ARENA_PROVIDER ||
      resolved.source.parser !== ARENA_PARSER_NAME ||
      resolved.source.parserVersion !== ARENA_PARSER_VERSION
    ) {
      throw new Problem(
        422,
        "parser_unsupported",
        "Source provider unsupported",
        resolved.source.provider ?? resolved.source.parser,
      );
    }
    const config = arenaConfig(resolved.source.config);
    const fetchUuid = uuidv7();
    await this.db.insert(ingestionSourceFetches).values({
      uuid: fetchUuid,
      sourceUuid: resolved.source.uuid,
      ownerUuid,
      credentialUuid: null,
      operationUuid,
      parserVersion: resolved.source.parserVersion,
    ...
    try {
      const bytes = await this.arena.fetchChannelCapture(config.channel_slug);
      const origin = await storeOwnedOriginArtifact(
        { db: this.db, blobs: this.blobs },
        {
          ...
          label: `arena-${config.channel_slug}-${fetchUuid}.json`,
        },
      );
      ...
      if (!fetched) throw new Error("Remote fetch state changed unexpectedly");

      phase = "parse";
      const parsed = await parseArenaCapture(bytes);
      if (parsed.channelSlug !== config.channel_slug) {
        throw new Error("Are.na returned a different channel than the configured source");
      }
      phase = "verify";
      const verify = verifyArena(parsed);
      if (!verify.ok) {
        throw new Error(`VERIFY rejected the Are.na channel: ${failedChecks(verify)}`);
      }
      phase = "candidate";
      const candidates = await arenaCandidatesFromParsed(
        parsed,
        resolved.source,
        origin,
        ownerUuid,
      ...
      const [verified] = await this.db
        ...
        .returning({ uuid: ingestionSourceFetches.uuid });
      if (!verified) throw new Error("Remote fetch state changed unexpectedly");
      return {
        candidates,
        fetchUuid,
        kind: "remote",
  ...
  private async stageCredentialSource(
    ...
    try {
      ...
      phase = "parse";
      const parser = parserFor(reservedSource.source.parser);
    ...
    await this.db.transaction(async (transaction: DatabaseTransaction) => {
      ...
      const remainingMemberships =
        policy === "replace"
          ...
          : membershipRows;
      let nextPosition =
        remainingMemberships.reduce((maximum, membership) => {
          return Math.max(maximum, membership.position);
        }, -1) + 1;
  ...
  private async persistCandidate(
    ...
    assertCandidate(
      candidate,
      candidate.owner.slice("rnet://id/".length),
      entry.origin_uuid,
      entry.elements,
    );
    ...
    await transaction.insert(mediaObjects).values({
      ...
      ownerUuid: candidate.owner.slice("rnet://id/".length),
  ...
  private async resolveSource(
    ...
    if (observedSource.kind === "remote") {
      ...
      assertPinnedParser(source);
      arenaConfig(source.config);
      return {
        kind: "remote",
        source,
        sourceStateDigest: await remoteSourceStateDigest(source),
      };
    }
...
function sourceUuidOf(source: string): string {
  const match = source.match(new RegExp(SOURCE_ID_PATTERN));
  if (!match) {
    throw new Problem(
      422,
      "schema_violation",
...
async function remoteSourceStateDigest(source: DbIngestionSource): Promise<string> {
  return digest({
    source: source.uuid,
    kind: source.kind,
    ...
    parser: source.parser,
    parser_version: source.parserVersion,
    config: arenaConfig(source.config),
  });
}

function arenaConfig(value: unknown): ArenaSourceConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("Stored Are.na source configuration is invalid");
  }
  const record = value as Record<string, unknown>;
  if (
    Object.keys(record).length !== 1 ||
    typeof record.channel_slug !== "string" ||
    record.channel_slug.length > ARENA_CHANNEL_SLUG_MAX_LENGTH ||
    !new RegExp(ARENA_CHANNEL_SLUG_PATTERN).test(record.channel_slug)
  ) {
    throw new Error("Stored Are.na source configuration is invalid");
  }
  return { channel_slug: record.channel_slug };
}
...
async function candidatesFromParsed(
  ...
  for (const transaction of parsed.transactions) {
    ...
    if (!validation.ok) {
      throw new Error(
        `Parser produced a nonconformant candidate: ${validation.issues
...
async function arenaCandidatesFromParsed(
  parsed: ParsedArenaChannel,
  source: DbIngestionSource,
  origin: DbOriginArtifact,
  ownerUuid: string,
  operationUuid: string,
  ...
  for (const block of parsed.blocks) {
    const objectUuid = uuidv7();
    const objectUri = `rnet://object/${objectUuid}`;
    const elements: StagedElement[] = [];
    for (const element of block.elements) {
      assertParsedArenaElement(element);
    ...
    const candidate: MediaObject = {
      ...
      keys: {
        ...block.keys,
        arena_block_id: block.blockId,
        arena_channel_id: String(parsed.channelId),
      },
      source: {
        ingest: { method: "parser", reproducible: true, skill: ARENA_PARSER_VERSION },
        origins: [`rnet://origin/${origin.uuid}`],
        retrieved_at: parsed.retrievedAt,
        properties: {
          ...block.sourceProperties,
          arena_block_type: block.blockType,
          arena_channel_slug: parsed.channelSlug,
          arena_channel_title: parsed.channelTitle,
    ...
    if (!validation.ok) {
      throw new Error(
        `Are.na parser produced a nonconformant candidate: ${validation.issues
          .map((issue) => `${issue.instancePath} ${issue.message}`)
          .join(", ")}`,
      );
    }
...
function assertParsedArenaElement(element: ParsedArenaElement): void {
  if (
    element.byteSize !== element.bytes.byteLength ||
    !/^sha256:[a-f0-9]{64}$/.test(element.contentHash)
  ) {
    throw new Error("Are.na parser produced inconsistent element metadata");
  }
}
...
async function identityForCandidate(candidate: MediaObject): Promise<string> {
  const arenaBlockId = candidate.keys?.arena_block_id;
  if (candidate.type === "arena.block" && arenaBlockId) {
    return digest({ type: candidate.type, arena_block_id: arenaBlockId });
  }
...
/**
 * Identifies the canonical transaction values emitted by a source independently of the
 * capture event that supplied them. Connected sources mint a fresh OriginArtifact on every
 * fetch; including that origin URI here would make an unchanged transaction look changed on
 * every pull and force `replace` policy to churn object identities.
 */
