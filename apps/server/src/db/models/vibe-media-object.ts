...
export const vibeMediaObjects = pgTable(
  "vibe_media_objects",
  {
    ...
    mediaObjectUuid: uuid("media_object_uuid").notNull().references(() => mediaObjects.uuid),
    addedAt: timestamp("added_at", { withTimezone: true }).notNull().defaultNow(),
    position: integer("position").notNull(),
  },
  (vibeMediaObject) => [
    primaryKey({ columns: [vibeMediaObject.vibeUuid, vibeMediaObject.mediaObjectUuid] }),
    uniqueIndex("vibe_media_objects_position_idx").on(vibeMediaObject.vibeUuid, vibeMediaObject.position),
