...
Return a concise summary, one to eight lowercase slug tags, and confidence from zero to one.
Ground every statement in the supplied data. Type names are hints, not applicability filters.
Derive the summary independently on every run. A Vibe always has a summary: never return null.
