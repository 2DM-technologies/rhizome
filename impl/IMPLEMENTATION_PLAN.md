...
A push task is a **triple: a name, a prompt, and an output schema.** Tasks live in `apps/server/src/push/tasks/{name}/`:
...
// ── 5. Operations ─────────────────────────────────────────────────
ops.push(vibeUri, { level, task, selection?, target?, refresh? }): Promise<Operation>
...
| M3   | Push pipeline               | `ModelConnector` interface + OpenAI connector (`gpt-5.6-luna`, batch or flex); the push task registry and the M3 task set defined in `impl/concepts/push-pipeline.md` (object- and Vibe-level tasks keyed on observed shape); write-back under `rhizome:{task}`; metering rows                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | inferred blocks present, costs queryable, connector swappable, per-run cost visible in `meter_entry`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
