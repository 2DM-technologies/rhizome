...
**`summarize` (Vibe).** One call over the `VibeContext` followed by the §5.6 per-object context of every distinct member in first-placement order. Output `{ title: string (1..256 characters, nonblank, single line), summary: string (pattern ^[\\s\\S]{1,600}$), tags: string[1..8] (pattern ^[a-z0-9][a-z0-9-]{0,31}$), confidence }`. `summary` is written to be reused as context by later Vibe-level runs (spec §2.4, §4.3). Runs first by convention; the host offers it first. If the context exceeds the per-call input ceiling, objects drop from the end in Vibe order and `result.context.truncated_objects` reports how many; summarize never fans out.
