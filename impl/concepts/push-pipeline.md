...
**Status:** Implemented. M3 ships the six tasks and the operation, metering, and host paths described here. §13 records the eleven-step commit sequence; §14 records the settled decisions D1–D35, including the owner-approved generation settings, context trimming, and image policy.
...
M3 ships six tasks (§5.3): `summarize`, `vibe-view`, and `vibe-orb` at the Vibe level; `display-name` and `search-keywords` at the object level; `describe-media` at the element level. Tasks see the **observed shape** of objects; `type` is a hint that prompts and rules may use, never a gate that keeps an object out of a task, because many Vibes will hold dMachine-defined, often polymorphic object types the store has never heard of.

Adding a task means adding its stable `(level, name)` reference to `PUSH_TASK_REFS`, adding a directory under `apps/server/src/push/tasks/{level}/` (manifest, prompt, output schema, optional rules), installing it in `installed-tasks.ts`, selecting it from any applicable source-skill pipelines, and regenerating the host's task list (§10). Task implementations and source skills consume the shared reference rather than repeating identity strings; the host is a typed consumer of the generated list (§10) that names the tasks it renders. The six M3 tasks between them cover every variation the manifest allows: element-, object-, and Vibe-level; text-only and image input; model-only (every dispatched chunk is a provider call) and rules-first (a deterministic `rules` function sees the `VibeContext`, decides clear-cut cases, and the model is called only when it returns nothing).
...
| connector swappable     | the integration and black-box suites run on `FakeModelConnector` injected through `AppDependencies`; the OpenAI connector is exercised only by stub-fetch unit tests; a grep test proves nothing outside `apps/server/src/inference/` and the test suites imports `inference/openai/` |
...
The loop drawn is the object-level path (`display-name`, `search-keywords`). An element-level task (`describe-media`) runs the same loop over the image elements of the selected objects, deduplicated by element UUID, and writes per element (§7.3). A Vibe-level task (`summarize`, `vibe-view`) takes the dotted branch and shares everything up to and including the connector call and the ledger. It differs in four places:
...
- a `rules` short-circuit, which only a Vibe-level task has, sees the whole `VibeContext` and can answer without a call (`vibe-view`);
...
apps/server/src/inference/
  ...
  openai/
    config.ts                   OPENAI_API_KEY, OPENAI_BASE_URL → OpenAIProviderSettings (§4.5)
...
No `openai` npm package: one endpoint, structured outputs, and a usage block are a few hundred lines with an injected `fetch`, and it keeps "swapping providers touches one file" literally true. `test/inference-boundary.test.ts` greps that nothing outside `inference/` imports `inference/openai/` (test files excepted, since the connector's own unit tests live with the suites) and that `config.ts` imports only `inference/config.ts`; the registry, the config loader, and the fake's use of the real rate card are the wiring `inference/` exists for. The registry lives in `inference/`, not `push/`: provider configuration, model naming, and usage normalization are provider wiring, not push logic.
...
export interface CompletionRequest {
  ...
  schemaName: string; // provider-facing label for the format, [A-Za-z0-9_-]{1,64}, e.g. rhizome_search-keywords
...
export interface ModelConnector {
  ...
  countTokens(
    i: Pick<CompletionRequest, "target" | "instructions" | "input" | "attachments">,
  ): Promise<number>; // packing estimate only
...
apps/server/src/push/
  ...
  tasks/                # one directory per level, then per task: identity is (level, name)
    vibe/
      ...
      vibe-view/        manifest.ts  PROMPT.md  output.json  rules.ts
    object/
      display-name/     manifest.ts  PROMPT.md  output.json
      search-keywords/  manifest.ts  PROMPT.md  output.json
    element/
      describe-media/   manifest.ts  PROMPT.md  output.json
```
...
export interface PushTaskDefinition {
  ...
  modelOutput?: { schema: JSONSchema; decode: (output: TaskOutput) => TaskOutput }; // Vibe-only generation envelope; decode after metering, before outputSchema and context validation
...
`PushTaskCatalog` wraps the installed definitions, keyed by `(level, name)`: the same name may exist at more than one level (an object-level and a Vibe-level `search-keywords`), and the written key is `rhizome:{name}` on both because the record kind already tells them apart. It throws at load on a bad name, a duplicate `(level, name)`, an element-level task without `elementKinds` or with a kind the attachment path does not accept, `rules` on a task below Vibe level, AJV compile failure, strict-subset violation, or empty prompt, and exposes two methods: `get(level, name)` for the pipeline, and `manifests()`, the serializable subset (`level, name, label, description, output_schema`) that `GET /push-tasks` returns and the host's generated task list is built from (§10).

All six tasks use `effort: "low"`. Their `outputTokens` settings are `{ base: 1024, perObject: 0 }` for `summarize` and `vibe-view`, `{ base: 1536, perObject: 0 }` for `vibe-orb`, `{ base: 128, perObject: 64 }` for `display-name`, `{ base: 128, perObject: 512 }` for `search-keywords`, and `{ base: 128, perObject: 1024 }` for `describe-media`. Batches use the policy default of 25 records; no task overrides it.
...
interface VibeContext {
  ...
  }>; // per type: how many, the observed pointers with leaf kinds, deduplicated, and element presence; at most 64 types and 512 pointers in total, and the serialized context at most 64 KiB; what was cut is counted in result.context and stated in the prompt
  elements: Array<{ kind: ElementKind; count: number }>; // elements in the Vibe by kind
}
```

`types[].count`, `types[].elements[].objects`, and `elements[].count` are what rules key on; `types[].pointers` is what lets `vibe-view` and its rules name real properties without seeing every object.
...
**`summarize` (Vibe).** One call over the `VibeContext` followed by the §5.6 per-object context of every distinct member in first-placement order. Output `{ title: string (1..256 characters, nonblank, single line), summary: string (pattern ^[\\s\\S]{1,600}$), tags: string[1..8] (pattern ^[a-z0-9][a-z0-9-]{0,31}$), confidence }`. The title pattern uses no lookaround; a pre-write post-check rejects leading/trailing whitespace, including a final newline that JavaScript’s `$` anchor can otherwise permit. `summary` is written to be reused as context by later Vibe-level runs (spec §2.4, §4.3). Runs first by convention; the host offers it first. If the context exceeds the per-call input ceiling, objects drop from the end in Vibe order and `result.context.truncated_objects` reports how many; summarize never fans out.

**`vibe-view` (Vibe).** Chooses the host surface for the Vibe and the configuration that surface needs. `output.json` is a root object with a discriminator and a config union:

```text
{ view: enum[datatable, mediaboard, simplelist, tweetfeed], config: anyOf[
...
**Object-level tasks** receive, per object: `type`, `source.properties` (strings cut at 8192 units), `user.properties` (owner annotations are authoritative), element metadata (kind, role, the element's `alt`, and the element's own `rhizome:*` entries such as a `describe-media` caption and description; never bytes or URIs), and `notes` = the object's own store-authored `rhizome:*` entries other than the task's own. Nothing from the Vibe. One object's serialized context is capped at 32 KiB: strings are cut first, then trailing properties drop, including notes and element metadata when necessary. The aggregate number of clipped objects appears in the input context and `result.context.clipped_objects`; prompts acknowledge that count without claiming that a per-record clipped flag exists. The 64 KiB `VibeContext` cap removes trailing observed pointers first, then trailing type groups; objects belonging to removed groups contribute to `truncated_objects`.
...
export type PushOperationResult =
  ...
  | (Shared & {
      ...
      vibe:
        ...
        | { outcome: "skipped"; reason: SkipReason; code?: ModelConnectorErrorKind }; // the run never reached a write: call_failed with the kind, aborted, invalid_output, or a durable entry that landed mid-run
    });
  ...
  anyOf: [
    {
      ...
      properties: {
        level: { const: "object" },
        // A bare task name, lowercase kebab-case ("search-keywords"); TASK_PATTERN comes from
...
Image payload reads observe the same operation wall signal, through both the storage request and response-body consumption. A stalled download therefore finalizes `aborted` with `max_wall`, preserving prior writes and metered usage, and lets the automatic sequence proceed to its next task. This does not change the attachment limits or add retries or restart recovery.
...
Not in M3: a jobs table, lease columns, a worker binary, or periodic scheduling. The owner-requested import trigger below uses the same in-process lifecycle. Also not in M3, and recorded in CONFORMANCE's M7 entry as open until durable execution: fencing a stranded run's late writes (a run cannot be stranded while its process is alive, and a dead process writes nothing); guarding a write against a `user` edit or a membership change between assembly and write; and metering a provider call in flight when the process dies.

### 6.4b Automatic enrichment after reviewed imports

After a successful reviewed import confirmation, the route queues `runImportedVibeTasks` in the API process and returns the Vibe immediately. Each source skill declares its task dependency graph; the installed catalogs validate it at startup. Ready independent nodes run together, and a dependent node accepts fresh context after its prerequisites finish their writes and finalization. Transaction sources declare empty graphs. Manual tasks retain their own applicability rules.

Automatic graphs serialize per Vibe. A second import waits until the previous graph has finalized and cleaned up its activity before starting. Different Vibes proceed independently. Empty graphs never replace or clear another graph's activity. The queue is process-local and is discarded on restart; no worker, jobs table, lease, or recovery scheduler is introduced.

An import into an existing Vibe enriches only its confirmed additions at object/element level, while Vibe-level tasks use the expanded collection. Element selection resolves live attached elements when its node starts. An empty additions list starts no tasks. Existing Vibe titles are preserved. Every task uses the shared acceptance checks, workset limits, durable protections, meter and finalizer. A failed task does not undo its import or completed siblings.

For a new Vibe without a source-provided name, only the standard `Imported objects` placeholder is eligible for summary naming. Other confirmation titles are explicit and remain unchanged. Naming reads the summary revision written by that graph's own operation, then conditionally updates the Vibe only while its current title still equals the confirmation placeholder. This atomic title comparison preserves an owner edit made during enrichment. Naming adds an owner-attributed Vibe revision and no extra model call. A missing, failed or invalid summary leaves the initial title. Manual summarize changes only inferred data.

Preview, cancellation, rejected/replayed confirmation, ordinary Vibe creation, and subsequent pulls do not trigger this graph. A store with no connector still commits imports without creating push operations. A restart interrupts any running graph: the existing boot sweep handles accepted unfinished operations; nodes not yet accepted are not resumed. M7's durable execution and membership freshness deferrals remain open.

### 6.5 Result and visibility

A completed push's `result`, as returned inside the operation document by `GET /operations/{id}`, for a `search-keywords` run over 8 selected objects (one was already durable, so 7 were sent):
...
Automatic import graph nodes wait for a conflicting same-task run to reach terminal state before releasing dependent nodes. After the wait, object/element nodes accept only records not already written, preserved (including a durable entry saved during the run), or classified as not applicable by that run; failed records remain eligible. A Vibe-level result covers the node only when its captured membership includes the graph's requested members. A reused summary does not become the graph's own naming result. Reacceptance checks permissions, live membership, and durability again. Automatic object/element nodes discard additions that are no longer eligible and continue with the remaining records; an empty selection completes without creating an operation. Explicit manual selections still reject ineligible records. This is bounded, process-local best-effort waiting: check the terminal row every 250 ms until the conflicting operation's creation time plus its wall limit and 60 seconds. A stranded queued/running row is then marked failed/interrupted, conditional on it still being active, and the waiting node reports an error. It does not add restart recovery or a durable job queue. Manual requests continue to receive the public 409 response without internal workset or operation details while a task is running.

Membership changes and manual object-identity pushes schedule derived orb refreshes in the background after their writes commit. Their HTTP response or terminal operation does not wait for a conflicting orb run. Refreshes coalesce per Vibe: at most one waits/runs, with one further pass for changes arriving during it. Unlike import graph coverage reuse, a derived refresh always composes fresh context after a conflict, because the covering run may predate saved identities or membership changes.
...
| rule-decided, no call (`vibe-view` rules)                                      | the configured identity | the rule identifier (`rhizome/vibe-view-rules@1`) | the rule identifier     |
...
- **Playwright:** existing gate unchanged plus `m3-push.e2e.ts` against the mock store, covering a Vibe task and an object task from step 9 and `describe-media` from step 10, and asserting the object and element queries refresh.
...
2. config: optional inference block with `inference/openai/config.ts`, the settings loader `loadInferenceConfig` calls, `.env.example`, README paragraph, config tests
3. inference: `ModelConnector`, strict-subset check, `inference/openai/rate-card.ts` (data and a pure pricing function, no HTTP, so the fake prices with the real card from the start), fake connector, registry serving the fake and validating the default target (with `openai` settings and no `useFake` it returns undefined until step 4, which nothing can observe before step 6), boundary test
4. inference: OpenAI Responses connector with flex tier, retries, usage mapping, and the registry's `openai` → `OpenAIConnector` mapping
...
7. tasks: `vibe-view` (with rules), `display-name`, `search-keywords`
8. semantics: push rules in `rnet-semantics.test.ts`
9. host: generated `push-tasks.ts`, push queries, task control, `vibe-view` → surface mapping, mock-store handlers, `m3-push.e2e.ts` for the object and Vibe cases
10. inference + tasks + host: image attachments on `CompletionRequest`, OpenAI `input_image`, `describe-media`, then the regenerated task list, the element task control, its mock, and the `describe-media` E2E case (last, so everything before it can merge if it slips)
...
| D8  | `vibe-view` views                                               | `datatable`, `mediaboard`, `simplelist` to start, each with config (§5.3)                                                                                                                                   |
...
| D30 | Vibe run outcomes                                               | `vibe.outcome` is `written`, `preserved_durable`, or `skipped` with a closed reason; never null (§6.1, §6.5)                                                                                                |
| D31 | Provider boundary                                               | Nothing outside `inference/` except test files imports `inference/openai/`; the registry, config loader, and fake's rate card are that wiring (§4.1)                                                        |
| D32 | Steps 2 to 4                                                    | The settings loader lands in step 2 and the rate card in step 3, so the fake and registry need no stubs; the connector and its registry mapping land in step 4 (§13)                                        |
| D33 | Task generation settings                                        | Every task uses low effort; output-token base/per-record values are recorded in §5.1; the default chunk size is 25.                                                                                         |
| D34 | Context trimming                                                | The 64 KiB Vibe cap drops trailing pointers then trailing type groups and counts removed-group objects; the 32 KiB object cap clips strings before trailing properties, notes, and element metadata (§5.6). |
| D35 | Image processing and estimation                                 | Fixed `detail: "high"`, Luna's published 32px patch formula, and the planned attachments passed to `countTokens`; estimation remains inside the inference boundary (§5.4).                                  |
...
Image processing was verified on 2026-09-10 against [Images and vision](https://developers.openai.com/api/docs/guides/images-vision#patch-based-image-tokenization): Luna high-detail sizing, patch coverage, resizing adjustment, and the 1.2 multiplier. A 1024 × 1024 image estimates 1229 image tokens; a 2048 × 2048 image is reduced to 1600 × 1600 and estimates 3000.
