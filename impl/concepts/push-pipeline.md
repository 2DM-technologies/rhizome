...
| the request's `AbortSignal` fires                                | `aborted`                                                    |
...
- `startPush` does all synchronous work (auth, task, workset, operation row, ledger open); `runPush(operationUuid)` rehydrates from the `operations` row alone (task, the resolved selection and elements all live in `request`), so a worker can call it with nothing but the id; records and the Vibe are loaded live, never snapshotted.
