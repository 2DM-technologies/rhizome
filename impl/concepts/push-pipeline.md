...
**Status:** Implemented. M3 ships the five tasks and the operation, metering, and host paths described here. §13 records the eleven-step commit sequence; §14 records the settled decisions D1–D35, including the owner-approved generation settings, context trimming, and image policy.
...
All five tasks use `effort: "low"`. Their `outputTokens` settings are `{ base: 1024, perObject: 0 }` for `summarize` and `vibe-view`, `{ base: 128, perObject: 64 }` for `display-name`, `{ base: 128, perObject: 512 }` for `search-keywords`, and `{ base: 128, perObject: 1024 }` for `describe-media`. Batches use the policy default of 25 records; no task overrides it.
...
The automatic path carries forward the owner's transaction exclusion: a Vibe containing any `transaction` object, including a mixed Vibe, is excluded from automatic inference. This is an automatic-import policy, separate from the shape-based applicability of manually invoked tasks. A store with no inference connector still commits imports and creates no automatic push operations.

`runAllTasks` iterates the installed catalog in element → object → Vibe order, preserving registration order within a level (summary before view). Each task is accepted only after the preceding task finalizes, so its context includes earlier results and its wall deadline starts with its own acceptance. Acceptance is shared with manual `startPush`; each task has the same resolved workset, duplicate guard, limits, operation row, metering, writer protections, and result serialization. A rejected or failed task does not prevent the remaining tasks from running or undo the import. Acceptance/finalization failures are logged without provider response bodies.

Owner-approved naming follow-up (2026-09-12): `summarize` also returns a short `title` in the same metered call. Once the automatic sequence completes, the import flow reads the confirmed preview's existing `destination` evidence. A source-provided title is kept; otherwise the summary's title is applied through the ordinary owner `VibesService.updateVibe` path, including a Vibe revision. No extra model call or task is added. Failed or invalid summaries and keyless/transaction-excluded imports keep their initial title. Manual summarize runs still write only inferred data. No concurrent-title-edit guard is added, as requested by the owner.
...
- **Automatic import follow-up (`push.integration.test.ts`):** confirmation commits before a gated fake connector is released and returns while inference is pending; a replay triggers no second sequence; all five installed tasks persist inferred results; unconfirmed, unauthorized, invalid, and failed-VERIFY imports create no tasks; ordinary creation, later imports, and pulls do not trigger the sequence; keyless imports succeed without tasks; transaction-only and mixed transaction Vibes are excluded; level ordering provides each task the prior tasks' results and a separate closed meter; failed tasks and workset rejections do not stop later levels.
