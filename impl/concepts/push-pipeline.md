...
| Host `useOperation` refreshes only on `committed_at`                                                                                                            | Push sets `committed_at` when ≥ 1 entry landed, §6.4                                                    |
...
flowchart TD
  subgraph accept["startPush · synchronous, no tokens spent"]
    A1["POST /rnet/v0/vibes/{id}/push<br/>{ level, task, selection?, target?, refresh? }"] --> A2["assertVibeScope(push)"]
    ...
    A6 --> A7["one transaction<br/>vibes FOR UPDATE · 409 if same (vibe, level, task) active<br/>INSERT operations (queued) · INSERT meter_entry (zeros)"]
    A7 --> A8["202 Operation"]
  end
  ...
  subgraph run["runPush(operationUuid) · rehydrates from the operations row"]
    R1["status = running"] --> R2["load objects in position order<br/>freshness · pack chunks"]
    ...
    R6 --> R7["write per object<br/>FOR UPDATE · durable guard · revision + operation_uuid"]
    R7 --> R3
...
The accept path is cheap and spends nothing; a run is a loop of assemble, complete, record, write, with the ledger updated before every write so a crash loses a write, never a charge. The run rehydrates from the `operations` row alone (§6.4a).
...
- one call, never chunked: the `VibeContext` (§5.2) followed by every selected object's per-object context, and oversize context truncates from the end rather than fanning out;
...
export function loadOpenAIProviderSettings(env): OpenAIProviderSettings | undefined; // OPENAI_API_KEY, OPENAI_BASE_URL, RHIZOME_OPENAI_SERVICE_TIER
```
...
export interface PushTaskDefinition {
  ...
  prompt: string;
  outputSchema: JSONSchema; // one result; authored in the JSON Schema subset the provider's strict structured-output mode accepts (§5.5), so the same file serves generation, re-validation, and discovery
...
`PushTaskCatalog` wraps the installed definitions, keyed by `(level, name)`: the same name may exist at more than one level (an object-level and a Vibe-level `search_keywords`), and the written key is `rhizome:{name}` on both because the record kind already tells them apart. It throws at load on a bad name, a duplicate `(level, name)`, AJV compile failure, strict-subset violation, or empty prompt, and exposes two methods: `get(level, name)` for the pipeline, and `manifests()`, the serializable subset (`level, name, label, description, output_schema`) that `GET /push-tasks` returns.

### 5.2 Shape, not type

Context assembly derives an `ObjectShape` for every object: its `type` string (whatever it is), the flattened key paths of `source.properties` and `user.properties` to a bounded depth (3) with the JSON kind at each leaf and arrays marked by item kind (`location.bed: string`, `photos[]: object`), its `keys` names, and its element kinds and roles. The shape is context for the model and the input to `VibeContext.types[].paths`; the model itself receives the full property JSON, nested values included, within the per-object size cap (§5.6). Applicability is the model's call, not a filter: every object in the selection is sent, and a task's prompt says when to return `null` for an object that does not make sense for it. `type` is one hint in the shape, never a gate, so a dMachine-defined `garden.plant` with an image element is as describable as a registered `transaction`. Vibe-level tasks and rules receive a `VibeContext`, the compact form of "the selection", computed by one grouped query plus the per-type union of object shapes:

```ts
interface VibeContext {
  title: string;
  summary: string | null; // rhizome:summarize.summary, omitted for summarize itself
  objects: number; // selected objects
  types: Array<{ type: string; count: number; paths: string[] }>; // per type: how many, and the union of observed property paths with leaf kinds, e.g. "source.properties.amount: number"
  elements: Array<{ kind: "text" | "image" | "audio" | "video" | "document"; count: number }>;
}
```

`types[].count` and `elements[].count` are the counts rules key on; `types[].paths` is what lets a task name real properties (a `dataframe` column, a display field) without seeing every object.

### 5.3 The M3 tasks

Every task writes the envelope `{ model, inferred_at, confidence?, properties }` under `rhizome:{task}`; `confidence`, when a task emits it, is lifted into the envelope rather than duplicated. Every `output.json` below is strict-subset (§5.5). Length limits are expressed as `pattern` (for example `^.{1,80}$`), so they are enforced at generation and nothing is clipped after the fact.

**`summarize` (Vibe).** One call over a selection of objects from the Vibe (default: all). The input is the `VibeContext` followed by the §5.6 per-object context of every selected object in position order (type, `source.properties`, `user.properties`, element metadata, notes). Output `{ summary: string (pattern ^[\\s\\S]{1,600}$), tags: string[1..8] (pattern ^[a-z0-9][a-z0-9-]{0,31}$), confidence }`. `summary` is written to be reused as context by every other task (spec §2.4, §4.3). Runs first by convention; the host offers it first. If the context exceeds the per-call input ceiling, objects drop from the end in Vibe order and `result.context.truncated_objects` reports how many; summarize never fans out.

**`vibe_view` (Vibe).** Chooses the host surface for the Vibe and the configuration that surface needs. The output is a discriminated union on `view`, one branch per surface, with the view-specific config inside it:

```text
dataframe:  { view, columns: path[1..8], sort: { path, direction: asc | desc } | null }
mediaboard: { view, caption_path: path | null, tile: square | natural }
simplelist: { view, title_path: path, subtitle_path: path | null }
```

Every `path` is an `enum` built per call from `VibeContext.types[].paths` (the same technique as refs), so the model can only name properties the Vibe actually has and an invented column is a schema violation at generation. Strict mode supports `anyOf`, so the union is expressed in `output.json` with the per-call enums spliced in by chunking; `properties` is written as `{ view, config }`. The `view` values are `VIBE_VIEWS` in `@rhizome/store-contract`, which the host imports for its surface mapping; a unit test asserts the `output.json` discriminator matches the constant so adding a view in one place only fails CI.

Rules first: the `rules` function sees the `VibeContext` and decides clear-cut cases from the counts and paths alone, filling the config deterministically; it may key on a registered type where that is the obvious answer (all `transaction` → `dataframe` with `columns = [posted_at, description, amount]` sorted by `posted_at` desc; all `arena.block` or `pinterest.pin` with image elements → `mediaboard`; one type, no elements → `dataframe` over its first eight paths). The model is called only when the rule returns nothing, which is a mixed or unfamiliar Vibe, so a plain transaction Vibe costs nothing. A rule-decided entry is written with `model` set to the rule's identifier (`rhizome/vibe_view-rules@1`) rather than a provider model, and no ledger call is recorded. The rule table lives beside the manifest and is unit-tested.

**`display_name` (object).** A short human name for the object, derived from its properties and element metadata; the prompt returns `null` for an object with nothing sensible to name. Output `{ display_name: string (pattern ^.{1,80}$) }`. An object has one name wherever it appears: the entry lives on the object, so two Vibes never render the same object differently, and the freshness rule (§6.3a) re-derives it when the object's properties change.

**`search_keywords` (object).** Document expansion for the host's lexical search. Output `{ keywords: string[1..20] (pattern ^[a-z0-9][a-z0-9 .'&-]{0,47}$) }`. PROMPT.md requires every term to be grounded in the object's properties, element metadata, or the Vibe summary (synonyms, spellings, and category words a person would type; no invented facts, no personal identifiers not already present). Indexed through a GIN over the inferred JSONB; embeddings, when they come, are a store-built index (rnet-spec-v0.1.md §2.4), not an inferred entry.

**`describe_media` (element).** The selection is still objects; the pipeline collects their `image` and `video` elements, deduplicated by element UUID, so a shared element is described once. An object with no such element contributes nothing and is reported `no_input`. `elements: "bytes"`: for each image element the pipeline fetches the payload through the existing `BlobStore` and passes it to the connector as an attachment. Video is skipped as `unsupported_media` in M3: frame extraction needs a worker the alpha does not have, and a video's `preview` image is its own element and is described as one. `output.json` is one result per element: `{ caption: string (one line, pattern ^.{1,120}$), description: string (dense prose of what is in the frame and how it is arranged, pattern ^[\\s\\S]{1,1200}$), medium: enum[photo, screenshot, illustration, diagram, document_scan, other], subjects: string[0..10] (objects, places, brands; people as roles, never identities), text_in_image: string|null (verbatim legible text) }`; chunking wraps it in the ref envelope (§5.5) with refs `e1..eN` over the attached elements only, so each attachment gets exactly one description and non-visual elements are never referenced. `caption` is what a list shows and `description` is what a later text-only run receives in place of the image; each result is written to its element as `inferred["rhizome:describe_media"]`. The element's own `alt` (source-provided, written at creation) goes into the call as context. Because an element's payload is immutable, the entry never goes stale: freshness is same model and same task only (§6.3a).

### 5.4 Connector extension for attachments

`CompletionRequest` gains an optional `attachments?: Array<{ ref: string; kind: ElementKind; mime: string; bytes: Uint8Array }>`. `elements: "bytes"` covers every element kind: the pipeline attaches `text` payloads as text, `image` payloads as image parts, and `document` payloads as file parts; `audio` and `video` are not attachable in M3 and are skipped as `unsupported_media`. The OpenAI connector maps them to `input_text`, `input_image` (base64 data URL, `detail: "low"` by default), and `input_file` content parts in the same user turn as the data block, counts their tokens from the provider's usage block like any other input, and prices them with the same rate card. `describe_media` is the only M3 task that attaches anything; text and document attachment is a live path with no consumer yet. A connector that cannot accept attachments throws `unsupported` before any request; `FakeModelConnector` records them. Per-attachment and per-call byte ceilings live in `push/limits.ts`.

### 5.5 Output schema authoring rules

One `output.json` serves generation, pre-write validation, and discovery unmodified. `assertStructuredOutputSchema` enforces the subset the configured connector supports, today OpenAI strict mode as verified on 2026-09-08: object root, `additionalProperties: false` on every object, every property in `required` (optionality is `["T","null"]`), allowed keywords `type enum const properties required additionalProperties items anyOf $defs $ref description pattern format(date-time,date,time,duration,email,hostname,ipv4,ipv6,uuid) minimum maximum exclusiveMinimum exclusiveMaximum multipleOf minItems maxItems`; forbidden `$schema $id title default examples minLength maxLength uniqueItems patternProperties allOf oneOf not if/then/else`. The concept is "the intersection the configured connector accepts", so a second provider's connector supplies its own check.

Object-level batching wraps `output.json` in an envelope per call: `{ results: [{ ref: enum[o1..oN], result: output.json | null }] }` with `minItems = maxItems = N` (strict mode supports both, so one answer per object is enforced at generation). `null` means the task does not make sense for that object; the pipeline writes nothing, reports `not_applicable`, and, if the object already holds a non-durable `rhizome:{task}` entry, removes it in the same locked write with a revision row. No entry is written for a `null`, so the object is re-evaluated on the next run. That is the only meaning of `null`, and every prompt states it; every other bad outcome is its own reason (§6.5): a result that fails `output.json` is `invalid_output`, a refused or truncated call is `call_failed` for the whole chunk. Refs are per-call ordinals, never URIs, so the model cannot address a record it was not given. Each `result` is re-validated against `output.json` after the envelope passes.

### 5.6 Context assembly and injection posture

Included: the `VibeContext` (§5.2), whose `summary` is `rhizome:summarize.summary` when present (except for `summarize` itself, spec §6.2 independent re-derivation); per object `type`, `source.properties` (strings cut at 512 units), `user.properties` (owner annotations are authoritative), element metadata (kind, role, the element's `alt`, and the element's own `rhizome:*` entries such as a `describe_media` caption and description; never bytes or URIs), and `notes` = store-authored `rhizome:*` entries other than the task's own. Excluded: `source.ingest`, `source.origins`, `keys` values, `x-*`, the task's own prior output, every entry from any other writer (free text from a partially trusted principal is the injection vector, and the store cannot tell a real correction from injected text at the write boundary), and `users.inferred`. The owner's own assertions reach the model through `user.properties`.
...
export const pushVibeRequestSchema = {
  ...
  properties: {
    ...
    // levels is never ambiguous; the host has both from discovery.
    ...
    refresh: { enum: ["stale", "all"], default: "stale" },
  },
};
...
4. Selection URIs → members of this Vibe, else 422 at `/selection/{i}`; omitted = whole Vibe in position order; objects with nothing to attach are dropped for `elements: "bytes"` tasks (`no_input`); then the freshness filter unless `refresh: "all"` (§6.3a); ≥ 1 eligible; ≤ `maxObjects` counted after both filters.
5. One transaction: `SELECT vibes FOR UPDATE`; an active push with the same `(vibe, level, task)` newer than `maxWallMs + 60 s` → 409 `operation_in_progress` (the age bound means a row stranded by a restart stops blocking once it could not still be running, so no reaper is needed); insert `operations{kind: push, status: queued, invokedBy: actor.subject, ownerUuid: vibe.ownerUuid, vibeUuid, request}` and `meter_entry{payer: "rhizome", model, zeros}`.
6. `queueMicrotask(runPush)` with the same catch-to-failed pattern as pull; the catch also closes the ledger.

Stored `request`: `{ mode: "push", level, task, target, refresh, selection | null, object_count, eligible_count, fresh_count }`.

### 6.3a Freshness

A whole-Vibe run must not re-derive what has not changed. An object's `rhizome:{task}` entry is **fresh** when all of these hold, and a fresh object is skipped with outcome `fresh` at no cost:

1. the entry exists;
2. its `model` equals the model identity this run resolved (a different target re-derives);
3. its `inferred_at` is later than the object's newest `source` or `user` revision (`media_object_revisions.created_at`), so a re-ingest or a `user.properties` edit invalidates it;
4. for tasks with `elements` other than `"none"`, later than the newest change to the object's element references.

An element-level entry is fresh when it exists and its `model` matches: an element's payload and metadata are immutable, so nothing else can invalidate it.

A Vibe-level entry is fresh under the same rule against the Vibe: same model, and `inferred_at` later than the newest `vibe_revisions` row and than every member's newest `source`/`user` revision. The check is one query over the revision tables, so an unchanged 500-object Vibe costs zero calls.

Freshness cannot see a change to the task itself (prompt or schema): the envelope has no field for a task version and `properties` is task output. `refresh: "all"` re-derives everything and is the operator's move after editing a task. Rule-answered entries follow the same rule with the rule identifier as `model`.

### 6.4 `runPush`

Status `running`; load and filter objects; pack chunks by count and estimated tokens (a single oversize object → `context_too_large`). Per chunk: check ceilings, assemble, `complete`, then `ledger.record` **before** the write (a crash between them loses the write, never the charge), unpack, re-validate per object, write per object. A chunk-level `output_*` or exhausted `provider_unavailable`/`timeout` marks that chunk's objects `call_failed` and the run continues; `auth` and `invalid_request` fail the run. Element level: the same loop over the selected objects' attachable elements, deduplicated by element UUID, writing per element. Vibe level: one call, then `writeVibeTaskInferred`. Finalize in one transaction: `operations{status, result, error, finished_at, committed_at = landed ? now : null}` (set whenever at least one entry landed, aborted runs included) and `meter_entry{duration_ms, abort_reason, breakdown}`. Statuses: `done` (possibly with skips), `aborted` (a ceiling; completed chunks stay written, each is an independent re-derivation), `failed`.
...
- `startPush` does all synchronous work (auth, task, target, selection, operation row, ledger open); `runPush(operationUuid)` rehydrates from the `operations` row alone (task, target, selection URIs all live in `request`), so a worker can call it with nothing but the id.
- Progress is resumable at chunk granularity: one write per object, ledger update per call, `breakdown.calls` indexed by chunk. A restarted run can skip objects whose `rhizome:{task}` entry is newer than the operation's `created_at`.
- The same-`(vibe, level, task)` guard is time-bounded, not lock-based, so a stranded row stops blocking without a reaper; a queue can replace it with a lease later.
...
A completed push's `result`, as returned inside the operation document by `GET /operations/{id}`, for a `search_keywords` run over 8 objects (`fresh` and `not_member` are decided before any call, so 6 were eligible; `failed` counts the `call_failed` entries):
...
```json
{
  ...
  "skipped": [
    { "uri": "rnet://object/0198f2a1-0000-7000-8000-000000000005", "reason": "fresh" },
    { "uri": "rnet://object/0198f2a1-0000-7000-8000-000000000006", "reason": "not_member" },
    { "uri": "rnet://object/0198f2a1-0000-7000-8000-000000000007", "reason": "not_applicable" },
    {
  ...
  "context": { "truncated_objects": 0 },
  "abort_reason": null
}
```

`vibe` and `elements` are the slots for the other two task levels and are `null` when unused, so every result has the same shape; `llm_calls` is the number of provider requests the run made (the same count as `meter_entry.turns`), `0` when a rule answered without the model.

`skipped[].reason` is one of: `not_applicable` (the model returned `null`; `removed: true` on the entry when an older non-durable entry was deleted as a result), `fresh` (§6.3a), `no_input` (a bytes task and the object has nothing attachable), `unsupported_media` (audio or video, not attachable in M3), `invalid_output` (the result failed `output.json` re-validation; strict mode should make this unreachable), `context_too_large` (one object exceeds the per-call input ceiling on its own), `call_failed` (the chunk's provider call failed, with the connector error kind as `code`), `not_member` (the object left the Vibe between accept and write), `aborted` (a ceiling stopped the run before this chunk ran). This list is a starting point and is extended during implementation as the code needs. For a Vibe-level task, `vibe` is `{ outcome: "written" | "preserved_durable" | "fresh", key, rev }`.

`OperationsService.getOperation` gains a `kind === "push"` branch asserting PUSH scope, so a push-only grantee can poll the handle it was returned; owner-only exposure already comes from `operations.owner_uuid`. The serializer redacts for non-owners: counts, `vibe.outcome`, `llm_calls`, `abort_reason`, and the URI lists stay; `usage` is visible only to the owner or the invoker (`operations.invoked_by`), since a dMachine that triggered a run should see what it cost even though the store pays; `error` collapses to a generic sentence (provider messages may echo prompt fragments).
...
// WriteOutcome = { outcome: "written"; rev } | { outcome: "removed"; rev } | { outcome: "preserved_durable" } | { outcome: "not_member" }
...
```
BEGIN
  locked = SELECT media_objects WHERE uuid = $1 FOR UPDATE          -- serializes with PUT /objects/{id}/inferred
  IF not a member of $vibe → ROLLBACK, not_member
  IF locked.inferred[key]?.durable === true → ROLLBACK, preserved_durable   -- CONFORMANCE M3
  next = { ...locked.inferred, [key]: entry }                        -- built from the LOCKED row, replaces only its key
...
Pushes with the same `(vibe, level, task)` are refused at accept (409). Different tasks or Vibes run concurrently; per-record `FOR UPDATE` serializes them with each other and with `setInferred`. Two runs of the same task: later commit wins (spec §6.2), the earlier snapshot stays in the revision log.
...
Minimal and generic, in this PR (decision D5): `queries/push.ts` (`usePushTasks` from discovery, `usePushVibe`, reuse `useOperation`); a task control on `VibeSurface` that lists discovered tasks by label, runs one, and polls; `rhizome:summarize` rendered on the Vibe; `ObjectSurface` keeps its raw inferred block. The host hard-codes no task names. `useOperation` already refreshes on `committed_at`, which push sets. Mock-store handlers for `GET /push-tasks`, `POST /vibes/{id}/push`, and a push branch in the operation mock, importing only `output.json` files (Playwright runs under Node and cannot load Bun text imports from `manifest.ts`); `apps/host/e2e/m3-push.e2e.ts` proves run → poll → refreshed inferred.

## 11. Tests

- **Unit (no DB):** strict-subset assertions over every installed `output.json` and a 25-ref envelope; OpenAI connector with injected `fetch`/`sleep`/`now` (request body, usage mapping, refusal/incomplete/invalid **with usage**, retry on 429 then 200, 4×503 → `provider_unavailable`, 401 no retry, never escalates tier, messages never contain the body); rate card (1M+1M at flex = `"0.700000"`, long-context flip, served tier overrides requested, unknown model throws); registry; fake connector; boundary grep; task catalog load and negatives; chunking and context rules (including "no summary in summarize's context" and an injection string serialized unchanged as data); `vibe_view` rule table and the `view` enum matching `VIBE_VIEWS`; attachment ceilings; freshness (fresh skip, invalidation on a `user` edit, `refresh: "all"`); catalog accepts one name at two levels and rejects a duplicate `(level, name)`; `level` required on the request; ledger accumulation and the constant payer; config; serializer redaction; store-contract schemas and OpenAPI ids/components.
- **Integration (`push.integration.test.ts`, Postgres + S3rver, fake injected):** `search_keywords` over a Vibe with an image-only object present → entries on the applicable objects, the rest `not_applicable`, revisions with `operation_uuid`, meter row with rate-card `usd`; same-key durable preservation (seeded via Drizzle); a `null` result removes an existing non-durable entry with a revision row; `describe_media` writes to elements with `media_element_revisions` rows; gated-fake concurrency with `PUT /inferred` landing mid-run; summarize writes the Vibe and validates; second summarize does not receive the first's summary; partial failure keeps chunk 1 and meters chunk 2; `auth` → `failed` with a closed ledger; ceilings → `aborted` with `max_turns`; a client-invoked push is metered with `payer = 'rhizome'` and `invoked_by = client:{name}`; 409 while active; 503 keyless after 403 still wins; foreign-Vibe selection → 422.
- **Black-box (`rnet-semantics.test.ts`):** push requires and names the `push` scope; push writes land only in `inferred` under the store's namespace; a push never replaces a durable entry or another writer's key; push output is never durable; a later run replaces only its own key; a push-only grantee can invoke and poll, a read-only grantee cannot invoke; task names are discoverable; an unlisted target is rejected with a pointer; a Vibe-level task lands in the Vibe and the Vibe still conforms; every push is metered; a store with no provider answers 503.
- **Playwright:** existing gate unchanged plus `m3-push.e2e.ts` against the mock store.
...
| D10 | Same-`(vibe, level, task)` 409 guard                            | Included                                                                                                 |
