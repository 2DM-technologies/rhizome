...
All five tasks use `effort: "low"`. Their `outputTokens` settings are `{ base: 1024, perObject: 0 }` for `summarize` and `vibe_view`, `{ base: 128, perObject: 64 }` for `display_name`, `{ base: 128, perObject: 512 }` for `search_keywords`, and `{ base: 128, perObject: 1024 }` for `describe_media`. Batches use the policy default of 25 records; no task overrides it.
