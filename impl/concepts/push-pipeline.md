...
**Object-level tasks** receive, per object: `type`, `source.properties` (strings cut at 512 units), `user.properties` (owner annotations are authoritative), element metadata (kind, role, the element's `alt`, and the element's own `rhizome:*` entries such as a `describe-media` caption and description; never bytes or URIs), and `notes` = the object's own store-authored `rhizome:*` entries other than the task's own. Nothing from the Vibe. One object's serialized context is capped at 2 KiB: strings are cut first, then trailing properties drop, including notes and element metadata when necessary. The aggregate number of clipped objects appears in the input context and `result.context.clipped_objects`; prompts acknowledge that count without claiming that a per-record clipped flag exists. The 8 KiB `VibeContext` cap removes trailing observed pointers first, then trailing type groups; objects belonging to removed groups contribute to `truncated_objects`.
...
Owner-requested follow-up (2026-09-12): after `POST /imports/{operation_id}/confirm` successfully commits a new Vibe and its reviewed contents, the route queues `PushService.runImportedVibeTasks` in the API process. Confirmation returns the existing Vibe document without waiting for inference. Preview, cancellation, rejected confirmation, ordinary Vibe creation, and subsequent pulls do not trigger this sequence; replaying confirmation still fails the existing consumed-preview check.

Owner-requested extension: confirmation of an import into an existing Vibe also queues the suite when it adds members. Confirmation returns the exact object URIs added by its transaction internally; this list is not an API field. Object tasks select only those additions. Element tasks resolve eligible attached elements when each task starts and deduplicate them; an empty element selection skips that task. Summary and view refresh against the expanded Vibe, without renaming it. Repeated imports that add no members start no tasks. Pending object/element status is limited to the added objects.

Every source-skill manifest declares a required, data-only `import_push_pipeline`; an empty array explicitly opts out. Stable task identities come from the shared `PUSH_TASK_REFS` constants used by both the task implementations and skill manifests, so skills never repeat task-name strings. Are.na and X select the common content-enrichment graph; OFX and SimpleFIN start with empty pipelines. During server construction, every source graph is compiled against the installed task catalog: missing tasks, repeated nodes, undeclared dependencies, and cycles fail startup. Confirmation resolves the import operation's source to its skill and executes that compiled graph. Core push orchestration never branches on a MediaObject `type`; a store with no inference connector still commits imports and creates no automatic push operations.

`runPushPipeline` schedules a node after every declared `after` dependency settles. Independent ready nodes run concurrently; in the common graph, `vibe-view` and `vibe-orb` start together after `summarize`. Each node accepts fresh context only when it becomes ready, so it sees its dependencies' completed writes and its wall deadline starts with its own acceptance. Edges order context enrichment rather than require success: a rejected, failed, or inapplicable node still unblocks its descendants, does not cancel siblings, and never undoes the import. Acceptance is shared with manual `startPush`; each task has the same resolved workset, duplicate guard, limits, operation row, metering, writer protections, and result serialization. Failures are logged without provider response bodies, and no automatic retries are added.

Owner-approved naming follow-up (2026-09-12): `summarize` also returns a short `title` in the same metered call. Once the automatic pipeline completes, the import flow reads the confirmed preview's existing `destination` evidence. A source-provided title is kept; otherwise the summary's title is applied through the ordinary owner `VibesService.updateVibe` path, including a Vibe revision. No extra model call or task is added. Failed or invalid summaries, empty pipelines, and keyless imports keep their initial title. Manual summarize runs still write only inferred data. No concurrent-title-edit guard is added, as requested by the owner.

Automatic scheduling adds no operation kind, push request/result field, or persistent queue. Results remain in the existing object, element, Vibe, and operation documents. The owner-approved status endpoint in §10 now provides live object/element loading state and revision-driven refresh on object pages; Vibe pages keep their existing refresh-on-focus/manual-refresh behavior for server-started runs. A process restart interrupts the sequence; accepted unfinished operations are handled by the existing boot sweep, and tasks not yet accepted are not resumed. M7's durable-execution deferrals remain open.
...
| D34 | Context trimming                                                | The 8 KiB Vibe cap drops trailing pointers before trailing type groups and counts removed-group objects; the 2 KiB object cap clips strings before trailing properties, notes, and element metadata (§5.6). |
