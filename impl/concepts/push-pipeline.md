...
This adds no operation kind, push request/result field, endpoint, or persistent queue. Automatic task results appear through the existing object, element, Vibe, and operation reads. The host keeps its existing refresh-on-focus/manual-refresh behavior for these server-started runs; it does not yet discover their handles for live progress. A process restart interrupts the sequence; accepted unfinished operations are handled by the existing boot sweep, and tasks not yet accepted are not resumed. M7's durable-execution deferrals remain open.
